...
// Emblema temático cuando no hay imagen (o falla la carga): un escudo con el
// icono del tipo de enfrentamiento y su etiqueta. No es un hueco vacío, sino una
// representación intencional de la batalla sin foto disponible.
export function ImageFallback({ type, label }: ImageFallbackProps) {
  const caption = label ?? (type ? TYPE_LABEL[type] : null)
  return (
    <div
      className="img-fallback"
      role="img"
      aria-label={caption ? `Sin imagen · ${caption}` : 'Imagen no disponible'}
    >
      <span className="img-fallback-emblem">
