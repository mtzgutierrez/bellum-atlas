...
import { wars, battles } from '../data/mock'
    ...
    <div className="ax-page">
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        ...
        <div style={{ padding: '32px 56px', display: 'flex', flexDirection: 'column', gap: 0 }}>
          {wars.map((w, i) => (
            <Link key={w.id} to={`/wars/${w.id}`} style={{
              ...
              <div>
                ...
                <div className="ax-mono" style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>{w.period}</div>
                <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 6, maxWidth: 480 }}>{w.summary}</div>
              </div>
              <div>
                <div className="ax-label" style={{ fontSize: 9 }}>Batallas</div>
                <div className="ax-mono" style={{ fontSize: 18, color: 'var(--color-text-primary)', marginTop: 4 }}>{w.battles}</div>
              </div>
              <div>
                <div className="ax-label" style={{ fontSize: 9 }}>Bajas est.</div>
                <div className="ax-mono" style={{ fontSize: 16, color: 'var(--color-text-primary)', marginTop: 4 }}>{w.casualties}</div>
              </div>
...
export function WarDetailDesktop() {
  ...
  const war = wars.find(w => w.id === id) ?? wars[0]
  const warBattles = battles.filter(b => b.war === war.name)
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        <section style={{ padding: '40px 56px 36px', borderBottom: '1px solid var(--color-border)', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 32, alignItems: 'start' }}>
          <div>
            ...
            <div className="ax-mono" style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginTop: 12 }}>{war.period}</div>
            <p style={{ marginTop: 16, fontSize: 15, color: 'var(--color-text-secondary)', lineHeight: 1.55, maxWidth: 560 }}>{war.summary}</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ['Duración', `${war.durationDays.toLocaleString()} días`],
              ['Batallas registradas', war.battles.toString()],
              ['Bajas estimadas', war.casualties],
            ].map(([l, v]) => (
              <div key={l} style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 12 }}>
                ...
                <div className="ax-mono" style={{ fontSize: 18, color: 'var(--color-text-primary)' }}>{v}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: '40px 56px' }}>
          <div className="ax-stamp" style={{ marginBottom: 20 }}>Batallas registradas · {warBattles.length > 0 ? warBattles.length : war.battles}</div>
          {warBattles.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {warBattles.map(b => <BattleCard key={b.id} battle={b} />)}
            </div>
          ) : (
          ...
          {wars.map((w, i) => (
            <Link key={w.id} to={`/wars/${w.id}`} style={{
              ...
              <div>
                ...
                <div className="ax-mono" style={{ fontSize: 10.5, color: 'var(--color-text-muted)', marginTop: 4 }}>{w.period} · {w.battles} batallas</div>
              </div>
